      ...
      for (unsigned int jy = 0; jy < num_div; ++jy) {
        if (abs_diff(ix, jx) <= 1 && abs_diff(iy, jy) <= 1) { // this grid is near to the particle as the grid indexes are close
          ...
          // use the center for the gravity of the grid : `acc.grid2cg[jy * num_div + jx]`
            Eigen::Vector2f center2grid = acc.grid2cg[jy * num_div + jx]; // the center for the gravity of the grid
            unsigned int np = acc.grid2idx[jy * num_div + jx + 1] - acc.grid2idx[jy * num_div + jx]; // the number of particles in the grid
            if (np == 0) { continue; } // skip empty grids
            particles[ip].force += static_cast<float>(np) * gravitational_force(center2grid - particles[ip].pos); // adding up forces from the representative particle which has all the weights of the particles in that grid
